...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: brand ? `Mi perfil | ${brand.name}` : "Mi perfil",
...
export default async function PerfilPage() {
  ...
  // Counts reales (comments + forum threads del user en esta brand).
  const [{ count: commentsCount }, { count: threadsCount }] = await Promise.all([
    supabase
      .from("comments")
      ...
      <section className="py-8 lg:py-12">
        <div className="mx-auto max-w-3xl px-6 lg:px-8">
          ...
          <div className="grid grid-cols-3 gap-4 mb-10">
            {[
              { icon: MessageSquare, label: "Comentarios", value: String(commentsCount ?? 0) },
              { icon: FileText, label: "Hilos creados", value: String(threadsCount ?? 0) },
